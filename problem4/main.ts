...
const result_a = sum_to_n_a(testN);
...
const result_b = sum_to_n_b(testN);
...
try {
  const result_c = sum_to_n_c(testN); // Try with `testN` first, reduce if it crashes
  ...
  console.error(`Error with sum_to_n_c (Recursion) for n=${testN}: ${error.message}`);
